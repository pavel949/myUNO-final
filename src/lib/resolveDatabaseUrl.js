 ...
 *
 * Each Vercel isolate must take at most one session-pooler client. The
 * pooler's `pool_size` here is 15; Prisma's default (`num_cpus * 2 + 1`)
 * exhausts it as soon as a handful of lambdas are warm
 * (`EMAXCONNSESSION`).
 *
 ...
 * @returns {boolean}
 */
function qualifyPoolerUsername(url) {
 ...
 * @returns {boolean}
 */
function capServerlessPool(url) {
  if (url.searchParams.has('connection_limit')) return false;
...
export function resolveDatabaseUrl(raw) {
  ...
  if (capServerlessPool(url)) changed = true;
