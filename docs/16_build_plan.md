...
**T-023b · Order finalisation: the confirm/dispute window.** `ServiceOrderStatus.closed` existed in the enum from T-023 but nothing ever wrote it, so a `fulfilled` order stayed disputable forever — an unbounded liability behind every provider remittance. Adds `[cfg] service.fulfilment_confirm_window_hours` (48, doc 07 F-PROV-3), `confirmServiceOrderFulfilment` (orderer closes early), the nightly `closeSettledServiceOrders` sweep (in the existing service-order job), and the matching refusal in `raiseDispute`. `closed_at`/`closed_by_identity_id` record how the order ended — a named identity confirmed, or null means the window lapsed. DoD: confirm→closed, sweep respects a per-project override, an open dispute holds the order open, a dispute past the window is refused, a never-fulfilled order stays disputable. Specs: 07 F-PROV-3; 04 §5; 13 §2. **Raised Q73** — remittance still pays on `fulfilled`, which the window does not align with.
