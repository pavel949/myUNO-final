...
interface CheckOutConditionReportModalProps {
...
export default function CheckOutConditionReportModal({
  bookingId,
  guestName,
  unitName,
  labels,
  onClose,
  onComplete,
}: CheckOutConditionReportModalProps) {
  const photoInputRef = useRef<HTMLInputElement>(null);
  const [checked, setChecked] = useState<CheckOutChecklistItem[]>([]);
  ...
  const toggleItem = (item: CheckOutChecklistItem) => {
    setChecked((current) =>
  ...
  const uploadPhotos = async (files: FileList): Promise<string[]> => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (!response.ok) {
        throw new Error(labels['staff.ops.checkout.photo_upload_error']);
      }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (checked.length === 0) {
      setError(labels['staff.ops.checkout.checklist_required']);
      return;
    }
    setBusy(true);
    setError(null);
  ...
  const title = labels['staff.ops.checkout.title']
      ...
      <div className="bg-surface-paper border border-border-line rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-24">
        <div className="flex items-start justify-between gap-16 mb-16">
          <div>
            ...
            <p className="text-small text-text-secondary mt-8">{labels['staff.ops.checkout.hint']}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          >
            {labels['staff.ops.checkout.close']}
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-24">
          <section>
            <h3 className="text-heading-3 font-semibold text-text-ink mb-12">
              {labels['staff.ops.checkout.checklist_title']}
            </h3>
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {CHECK_OUT_CHECKLIST_ITEMS.map((item) => (
                <li key={item}>
                  <label className="flex items-center gap-8 text-body text-text-ink cursor-pointer">
                    <input
                      ...
                      className="h-16 w-16"
                    />
                    {labels[`staff.ops.checkout.checklist.${item}`]}
                  </label>
                </li>
              ))}
            </ul>
          ...
          <section>
            <h3 className="text-heading-3 font-semibold text-text-ink mb-12">
              {labels['staff.ops.checkout.photos_title']}
            </h3>
            <p className="text-small text-text-secondary mb-12">
              {labels['staff.ops.checkout.photos_hint']}
            </p>
          ...
          <section>
            <label className="block text-heading-3 font-semibold text-text-ink mb-12">
              {labels['staff.ops.checkout.notes_title']}
            </label>
            <textarea
              value={notes}
              ...
              placeholder={labels['staff.ops.checkout.notes_placeholder']}
            />
          </section>

          <Button type="submit" isLoading={busy}>
            {labels['staff.ops.checkout.submit']}
          </Button>
        </form>
      </div>
    </div>
