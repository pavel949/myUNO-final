...
**T-014 · Payment seam + mock provider.** `finance` seam (createCheckout/confirm/refund/preauth), mock checkout page, idempotent confirm (return + webhook), Payment/Refund tables, ledger auto-entries on confirm. DoD: webhook idempotency test; refund lifecycle test; mock page drives a booking to `confirmed`. Specs: 10 §§1–2, 8; 02 §5.

**T-015 · Search & public booking UI.** Public pages: search (S3, filters from config catalogs, map), unit detail (S4) with calendar + BookingWidget, review-and-pay (S5) with auth-inline, request-to-book variant; trips list with hold countdown/complete-payment. DoD: e2e happy path register→search→book→mock-pay→trips shows confirmed; unhappy: expired hold, dates-taken race, below-min-nights. Specs: 07 F-GUEST-1..4; 06 S3–S5; 08 §1.
