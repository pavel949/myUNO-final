...
// Inline styles mirror the design tokens (brand.andaman #0E4F4B,
// surface.ivory #F5EFE4, text.ink #16211F, text.stone #7E8C88) — a route
// handler response has no Tailwind pipeline, and this tiny page must stand
// alone. Keep in lockstep with tailwind.config (doc 06).
export async function vanityNotFound(): Promise<NextResponse> {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>404 · ${labels['common.not_found.title']}</title></head><body style="margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;background:#F5EFE4;color:#16211F"><div style="max-width:28rem;padding:2rem;text-align:center"><p style="font-size:2.5rem;font-weight:700;color:#0E4F4B;margin:0 0 1rem">404</p><h1 style="font-size:1.25rem;margin:0 0 .75rem">${labels['common.not_found.title']}</h1><p style="color:#7E8C88;margin:0 0 1.5rem">${labels['common.not_found.body']}</p><a href="/" style="display:inline-block;padding:.75rem 1.5rem;border-radius:.375rem;background:#0E4F4B;color:#F5EFE4;text-decoration:none;font-weight:500">${labels['common.not_found.home']}</a></div></body></html>`;
