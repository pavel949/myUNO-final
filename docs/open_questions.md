...
### Q49. A provider editing their own service price writes baht into a satang column — OPEN
- **Source:** found while sweeping the satang/baht display bug (Q47) — this is the write-side mirror of it, in the same screen.
- **The bug:** `/provider/services` — both `create` and `saveEdit` — take whatever the provider types in the price field and send it straight through as `basePriceThb` with no `×100`: `basePriceThb: Number(price)`. If a provider means "my massage costs ฿800" and types `800`, the service is saved priced at **฿8** (800 satang). The edit form's pre-fill has the matching mirror bug — it shows the raw satang integer back as if it were baht (`String(service.basePriceThb)`), so a provider re-opening a correctly-priced ฿800 service sees "80000" in the box.
- **Why this needs a ruling, not a guess:** fixing it means deciding what the input box asks a provider for. Two shapes work: (a) keep the box in baht — multiply by 100 on submit, divide by 100 on pre-fill, or (b) tell the provider the box is satang (unlikely to be the intended UX for a person pricing a massage). Guessing wrong silently reprices every service a provider edits after the fix ships.
- **Not blocking today in the way Q47 was:** this is a data-entry bug, not a rendering one — no existing correctly-priced service is misdisplayed by it. But any service a provider has already created or edited through this form may already be priced 100× too low, and that needs a data check before go-live, not just a code fix.
- **Needs from founder/ops:** confirm the input should be baht (matching every other admin/provider money input in the platform, e.g. the cost-recording screen), then fix `create`/`saveEdit`/`startEdit` together so the round-trip is consistent, and audit existing `Service` rows for a price that looks 100× too low against what the provider likely intended.

### Q50. Setting a unit's NOI cap writes baht into a satang column — OPEN
- **Source:** found alongside Q49, same class of bug, different screen — the admin onboarding page's "set engagement" form.
- **The bug:** `/app/admin/units/[id]`'s onboarding form sends the admin's typed NOI-cap number straight through as `noiCapAnnualThb` with no `×100`: `noiCapAnnualThb: noiCap ? Number(noiCap) : undefined`. The same page's read-only list of existing engagements has the same bug — `` `฿${e.noiCapAnnualThb.toLocaleString()}` `` — no division, so an admin re-reading a cap they just set sees it 100× too large, matching what they typed, which makes the write bug easy to miss by eye.
- **Why this one is more consequential than Q49:** CLAUDE.md is explicit that "a direct-managed unit without its NOI cap refuses statement generation — no guessing," and doc 07's owner-statement flow compares this cap directly against satang NOI totals computed elsewhere (`statement.service.ts`). A cap entered as baht-typed-into-satang is not just wrong — it is wrong by exactly the factor that would make a real cap look 100× smaller than intended, silently changing which statements get capped.
- **Needs from founder/ops:** the same ruling as Q49 — confirm the box should read baht in, baht out — then fix the submit handler and the read-only list together, and check whether any live engagement already has a cap set through this form before it is trusted for a real statement run.
