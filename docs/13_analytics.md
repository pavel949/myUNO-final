...
**Project reports (LY-10, read-time — `reports.service.ts`, no new rollup dimensions):**
- **Occupancy by category** — booked nights (occupying statuses, clipped to the range) ÷ available nights (live units of the category × range nights); units without `category_key` report under `uncategorized`; draft/paused units are out of the availability base. *(Maintenance blocks not yet subtracted — matches the loop-one headline caveat above.)*
- **Revenue by channel** — ledger `rental_revenue` rows joined to their booking's `channel` (the ledger is the money truth); unattributed rows report as `unattributed`.
- **Rental vs ancillary split** — the brief's hard rule: **ancillary is a separate line**. Rental = ledger `rental_revenue`; ancillary = ledger `service_commission` (the platform's take on service orders). Surfaced per live project on the admin dashboard (last 30 days).
