...
interface PriceBreakdownProps {
  ...
  totalLabel: string;
  /** Server-computed total. When omitted, the visible total is the sum of lines. */
  totalSatang?: number;
}

export function PriceBreakdown({ lines, totalLabel, totalSatang }: PriceBreakdownProps) {
  ...
  return (
    <div className="border border-border-line rounded-md p-16 bg-surface-paper">
      ...
      <div className="flex justify-between gap-16 pt-12 mt-4 border-t border-border-line">
        ...
        <MoneyAmount satang={shownTotal} className="text-title font-semibold" />
      </div>
    </div>
  );
}
