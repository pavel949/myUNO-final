...
const map = JSON.parse(fs.readFileSync(join(__dirname, 'ops-flow-th-map.json'), 'utf8'));
const path = join(__dirname, '../src/modules/content/seed.ts');
const lines = fs.readFileSync(path, 'utf8').split('\n');

const namespaces = new Set(['tickets', 'orders', 'checkin', 'payments', 'messages', 'guests', 'account']);
...
console.log(`Updated ${updated} ops-flow keys`);
