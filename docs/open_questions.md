...
### Q47. Money is stored in satang and some screens display it as baht — OPEN
...
- **The problem:** `formatThb()` (`src/components/viz/palette.ts`) formats its argument as **baht**, and several screens hand it a satang figure directly — the admin ledger (`formatThb(entry.amountThb)` and its totals), and the admin dashboard's ADR / RevPAN / revenue tiles. If those figures are satang, every one of them is shown **100× too large**.
- **Why it was not simply "fixed":** the fix depends on what each *producer* actually stores, and the producers were not all verified. Dividing at a call site whose source is genuinely in baht would introduce the opposite error, on a finance screen, silently. Establishing the unit of every money field is a pass of its own and is worth doing deliberately.
