...
-- One BAR plan per category. project_id is intentionally NULL here because the
-- schema permits category-scoped plans and project_id/code has a unique index.
-- The pricing engine resolves Unit -> Category -> Project in that order.
...
BEGIN
  ...
  -- The FK is canonical. Keep the legacy alias synchronized during transition.
...
-- Abort migration rather than leaving a half-canonical live inventory.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM unit
