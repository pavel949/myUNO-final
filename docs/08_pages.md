...
| `/app/services`, `/app/services/{id}`, `/app/orders` | Marketplace + orders. *(SA-1, as built at `/services`: super-app facade — category icon grid from `catalog.service_categories` with tap-to-filter + `?category` deep link, and a stay-context banner when entered from the home space with `bookingId`.)* | S11 |
