...
- **Needs from founder:** (1) adopt them into `schema.prisma` (`StatementLineItem` model + the twelve `OwnerStatement` fields) and finish the line-items route, making the transparency requirement real; or (2) rule them out of loop one, in which case a migration should drop them so the schema and database agree again. Until ruled, they stay in place and unused.
