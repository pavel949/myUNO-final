...
describe('POST /api/service-orders/[id]/cancel — MC authorization', () => {
  ...
  beforeEach(async () => {
    ...
    const order = await db.serviceOrder.create({
      data: {
        ...
        quantity: 1,
        price_breakdown: { base_thb: 30_000, quantity: 1, total_thb: 30_000 },
        total_thb: 30_000,
  ...
  it('allows managed MC member to cancel a service order', async () => {
    asMcMember(managedOrgId);
    const response = await POST(
      new NextRequest(`http://localhost/api/service-orders/${orderId}/cancel`, {
        method: 'POST',
        body: JSON.stringify({ reason: 'ops_replan' }),
  ...
  it('blocks MC outside the managing organization', async () => {
