...
- **The scheduler** runs the jobs registry (`src/jobs/`, doc 14 §1). Vercel Hobby allows two cron slots, so the 5-minute hold-expiry cadence in the original list is packed with TM30 escalation and iCal import into `/api/cron/run-frequent` every **15 minutes**; nightly work (verification deadlines, retention/PDPA deletions, metric rollups, guest lifecycle sends, stale service-order expiry) runs at `/api/cron/run-all` (19:00 UTC / 02:00 ICT). Notification digests and monthly statement generation are still admin- or event-driven, not cron jobs — they are not in the registry until they are. Every registered job writes an append-only `job_run` row (last run + outcome). The admin panel at `/app/admin/scheduler` lists the whole registry even when a job has never run — a silent scheduler is a visible red light, not a mystery.
