...
- **Fixed widths (layout, not rhythm):** dimensions that are not on the 4-based scale still live in the theme, never in an inline style. `min-w.btn-sm 80 / btn-md 96 / btn-lg 112` are *floors* under the button sizes — a button grows with its content and is never measured off its English label (board 21: `Забронировать` runs ~86% longer than `Reserve`). `w.field-sm 140 / field-md 150 / field-lg 200` are the inline text inputs that sit in a table row; `w.panel 320` + `max-h.panel 360` is the notification popover; `w.thumb 72` is the unit-list thumbnail.
