...
| **Admin** | `/app/admin` | 15+ pages incl. payouts + reconciliation link | **Works** — doc 08 §6 sections built and linked in sidebar; reconciliation at `/app/admin/reconciliation` |
...
- ~~**F-FIN-2 (payouts and reconciliation)**~~ — **built:** `/app/admin/payouts` (in admin nav) records owner and provider payouts; `/app/admin/reconciliation` board linked from payouts; provider portal `/provider/remittances` (F-PROV-4).
...
- ~~**F-MC-2**~~ — **built (2026-09-04):** tabbed MC portal + `/mc/calendar` unit picker + `/mc/units/[unitId]` availability/pricing; fee report period picker + CSV export.
...
~~**Finance is built but invisible.**~~ **Fixed:** ledger, statements and payouts are now in the sidebar. Reconciliation now sits inside the admin group at `/app/admin/reconciliation`, in the Money & record section; the old `/admin/finance/reconciliation` address permanently redirects to it.
